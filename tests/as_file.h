...
// `START`, then `ITMT`. The tests were written before a file had a shape, and
// there are some four hundred of them, so the shape is put on here rather than
// typed out four hundred times — what each test is about is the program, not
// the wrapper.
//
...
inline std::string asFile(const std::string &text) {
  ...
  if (text.rfind("START", 0) != 0 && at == std::string::npos)
    ...
    // a file still has every block, so the ones that run are written empty.
    return "READ_ME { }\nPREP {\n" + text + "}\nSTART { }\nITMT { }\n";
